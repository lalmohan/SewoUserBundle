    ...
    public function process(Request $request, Form $form, UserResponseInterface $userInformation)
    {
        ...
        if ('POST' !== $request->getMethod()) {
            ...
            if ($userInformation instanceof AdvancedUserResponseInterface){
               $this->onSuccess($user, $userInformation); 
              } 
            $form->setData($user);
        }
...
*/
	...
	protected function onSuccess($user, $userInformation)
    {    
		$user_exist=$this->userManager->findUserByEmail($userInformation->getEmail());
		$resourceowner=$userInformation->getResourceOwner()->getName();
		if($user_exist){
			$user=$this->userManager->findUserByEmail($userInformation->getEmail());
